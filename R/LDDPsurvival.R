...
#' Default method for LDDPsurvival
#' @param formula survival formula
#' @param zpred prediction matrix
         ...
         work.dir=NULL)
{
       ...
       #########################################################################################
         ...
         ysave <- matrix(0,nrow=nsave,ncol=nrec)
       ...
       #########################################################################################
         ...
         save.state <- list(thetasave=thetasave,
                            randsave=randsave,
                            ysave=matrix(foo$ysave,nrow=nsave,ncol=nrec))
...
#' Print method for LDDPsurvival objects
#' @param x LDDPsurvival object
...
#' Summary method for LDDPsurvival objects
#' @param object LDDPsurvival object
#' @param hpd logical, whether to compute HPD intervals
#' @param ... additional arguments
#' @export
"summary.LDDPsurvival"<-function(object, hpd=TRUE, ...) 
{
    ...
    # HPD / equal-tail helpers without DPpackage Fortran dependency
    hpd_interval <- function(x, level = 0.95) {
         if (requireNamespace("coda", quietly = TRUE)) {
             ci <- coda::HPDinterval(coda::mcmc(as.numeric(x)), prob = level)
             return(as.numeric(ci[1, ]))
         } else {
             a <- (1 - level) / 2
             return(as.numeric(stats::quantile(x, probs = c(a, 1 - a), na.rm = TRUE)))
         }
    }
    
    hpdf<-function(x)
    {
         hpd_interval(x, level = 0.95)
    }
...
#' Print method for summaryLDDPsurvival objects
#' @param x summaryLDDPsurvival object
#' @param digits number of digits to print
#' @param ... additional arguments
#' @export
...
#' Plot method for LDDPsurvival objects
#' @param x LDDPsurvival object
#' @param hpd logical, whether to show HPD intervals
#' @param ask logical, whether to ask before plotting
#' @param nfigr number of figure rows
#' @param nfigc number of figure columns
